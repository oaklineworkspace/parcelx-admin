    ...
    <Stack gap="lg">
      ...
      {!supabaseConfigured && (
        <Alert icon={<IconInfoCircle size={16} />} title="Supabase Not Configured" color="blue">
          To connect to your database, add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to your environment variables.
        </Alert>
      )}
                ...
                <Card key={shipment.id} withBorder padding="sm">
                  <Group justify="space-between" wrap="nowrap">
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <Text fw={500} truncate="end">{shipment.tracking_number}</Text>
                      <Text size="sm" c="dimmed" truncate="end">{shipment.origin} → {shipment.destination}</Text>
                    </div>
                    <Badge 
                      color={
