  ...
  return (
    <Stack gap="lg">
      <Group justify="space-between" wrap="wrap">
        ...
        <Button leftSection={<IconPlus size={16} />} onClick={() => router.push('/shipments/new')}>
          Create Shipment
        </Button>
      </Group>
      ...
      {!supabaseConfigured && (
        <Alert icon={<IconInfoCircle size={16} />} title="Supabase Not Configured" color="blue">
          To manage shipments, add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY to your environment variables.
        </Alert>
      )}
                ...
                <Table.Tbody>
                  {data?.shipments.map((shipment) => (
                    <Table.Tr key={shipment.id}>
                      ...
                      <Table.Td>
                        <Group gap="xs">
                          <ActionIcon variant="light" onClick={() => router.push(`/shipments/${shipment.id}`)}>
                            <IconEye size={16} />
                          </ActionIcon>
